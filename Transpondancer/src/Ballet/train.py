...
train_loader, test_loader = dh.pre_processor('../Dataset', batchsize= batch_size)

classes = ('Arabesque', 'Grand_Plié', 'Pirouette')
...
for e in range(epochs):
    ...
    if e%2 == 0:
        ...
        print('Epoch : ',e, "\t Train loss: {:.2f}".format(running_loss/images.shape[0]),
            "\t Validation loss: {:.5f}".format(val_loss), "\t Accuracy: {:.2f} %" .format(acc))

        # Save the model only if the val_loss decreases w.r.t previous loss
        if val_loss < best_val:
            best_val = val_loss
            # PATH = '../../models/ballet_1000epochs_cnn.pth'
            PATH = '/Users/alisaavdic/Desktop/THESIS/Transpondancer/models/ballet_retrained_retire_2.pth'
